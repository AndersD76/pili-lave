...
static lv_obj_t* btn_cancelar_auto  = nullptr;
...
// Habilita CANCELAR sempre que houver algo pra cancelar (qualquer coisa
// além de IDLE — aguardando carro, processando, pausado, erro, etc.).
// Chamar SEMPRE com o lock do LVGL em mãos.
void tela_auto_atualizar_cancelar() {
    if (!btn_cancelar_auto) return;
    if (g_estado_auto == AUTO_IDLE) lv_obj_add_state(btn_cancelar_auto, LV_STATE_DISABLED);
    else                            lv_obj_clear_state(btn_cancelar_auto, LV_STATE_DISABLED);
}

// CANCELAR — aborta a lavagem em andamento (qualquer estado) e libera a
// máquina de volta pro IDLE, pronta pra receber outra ordem do app.
static void cb_cancelar_auto(lv_event_t* e) {
    if (g_estado_auto == AUTO_IDLE) return;   // nada pra cancelar
    Serial.println("[AUTO] CANCELAR (botao) — abortando lavagem");
    // Avisa a nuvem/app (evento de fault, não wash-complete — cancelamento
    // não é conclusão, não pode gerar cobrança) ANTES de zerar o estado, pra
    // o reservationId ainda salvo (nvs_get_last_res_id) ir junto no evento.
    backend_evt_fault("Cancelado pelo operador");
    auto_cancelar();
    // Apaga a lâmpada de vez (não deixa a nuvem reacender com o lightState
    // antigo até o heartbeat seguinte confirmar o cancelamento — ~10-15s de
    // folga é mais que suficiente pra isso).
    lampada_app_forcar_off_temporario(15000);
...
void tela_auto_criar(void (*cb_voltar)()) {
    ...
    // CANCELAR LAVAGEM (abaixo do LIMPAR ERRO) — aborta o ciclo em qualquer
    // estado (aguardando carro, processando, pausado...) e libera a máquina
    // de volta pro IDLE. Desabilitado quando já está em IDLE (nada a cancelar).
    lv_obj_t* btn_cancel = lv_btn_create(scr_auto);
    lv_obj_set_pos(btn_cancel, 660, 70);
    lv_obj_set_size(btn_cancel, 130, 50);
    lv_obj_set_style_bg_color(btn_cancel, lv_color_hex(0xFF8F00), 0);   // laranja — distingue de LIMPAR ERRO
    ...
    lv_obj_add_event_cb(btn_cancel, cb_cancelar_auto, LV_EVENT_CLICKED, nullptr);
    btn_cancelar_auto = btn_cancel;
