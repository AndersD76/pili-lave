...
// -----------------------------------------------------------------------
// Navegação (definida em display.ino) — exposta pra backend_client.h poder
// levar a tela pra TELA_AUTO quando a nuvem autoriza uma lavagem, sem
// precisar do enum TelaAtiva (que só existe depois, no .ino).
// -----------------------------------------------------------------------
void ir_tela_auto_se_preciso();   // no-op se já estiver na TELA_AUTO

// Definida em lampada_app.h — usada por tela_auto.h (botão CANCELAR), que é
// incluído antes de lampada_app.h.
...
// Waveshare — leitura FC02 em blocos de 8 bits (protocolo com latch por
// interrupção, read-and-clear): start=0 -> estado AO VIVO das 8 DIs;
// start=8 -> LATCH (pulsos desde a última leitura, zera ao ler).
#define MB_DI_VIVO       0
#define MB_DI_LATCH      8

// Registradores Waveshare (discrete inputs) — legado, endereço por DI
// individual; não usado pelo protocolo em blocos acima.
...
// Pacote de sincronização de canal (câmera/display -> Waveshares e vice-versa
// no resgate). Movido de wifi_manager.h pra cá: comm_espnow.h (incluído ANTES
// de wifi_manager.h) também precisa dele, pro resgate de waveshare perdida.
typedef struct __attribute__((packed)) {
    CabEspNow cab;
    uint8_t   canal;
} MsgCanal;
...
typedef struct __attribute__((packed)) {      // Display -> Câmera
    ...
    char      errcode[64]; // código/msg do erro (só usado em fault; "" senão) — era
                            // 24, curto demais: truncava mensagens reais tipo "timeout
                            // GIRO2 (X17+HOME nao confirmou)" (40+ chars)
} MsgEvt;
...
// ----- Lista de redes Wi-Fi (tela de config) — só a câmera escaneia; o
// display nunca mexe no próprio rádio pra isso, só fica no canal 1 recebendo
// esta lista paginada. DEVE bater byte a byte com o firmware da câmera.
typedef struct __attribute__((packed)) { CabEspNow cab; } MsgScanReq;   // Display -> Câmera
