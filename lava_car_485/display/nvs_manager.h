...
#define NVS_PROG1        "prog1"        // uint32 — lavagens prog1 desde acerto (total, app+presencial)
#define NVS_PROG2        "prog2"
#define NVS_PROG3        "prog3"
#define NVS_PROG4        "prog4"
// Quebra por origem (só pra exibição na tela — não mexe no saldo/acerto,
// que continua somando os totais acima). "Presencial" = iniciado no botão
// físico X1-X6; "App" = iniciado pelo aplicativo (tem reservationId).
#define NVS_PROG1_PRES   "p1_pres"      // uint32
#define NVS_PROG2_PRES   "p2_pres"
#define NVS_PROG3_PRES   "p3_pres"
#define NVS_PROG4_PRES   "p4_pres"
#define NVS_PROG1_APP    "p1_app"
#define NVS_PROG2_APP    "p2_app"
#define NVS_PROG3_APP    "p3_app"
#define NVS_PROG4_APP    "p4_app"
...
// -----------------------------------------------------------------------
// Contadores por programa, quebrados por origem (presencial / app) — só
// pra exibição na lista da tela; o total (nvs_get_prog acima) continua
// sendo a soma dos dois e não muda de comportamento.
// -----------------------------------------------------------------------
static const char* _nvs_prog_key_pres(int prog) {
    switch (prog) {
        case 1: return NVS_PROG1_PRES;
        case 2: return NVS_PROG2_PRES;
        case 3: return NVS_PROG3_PRES;
        case 4: return NVS_PROG4_PRES;
        default: return NVS_PROG1_PRES;
    }
}
static const char* _nvs_prog_key_app(int prog) {
    switch (prog) {
        case 1: return NVS_PROG1_APP;
        case 2: return NVS_PROG2_APP;
        case 3: return NVS_PROG3_APP;
        case 4: return NVS_PROG4_APP;
        default: return NVS_PROG1_APP;
    }
}
uint32_t nvs_get_prog_pres(int prog) { return _nvs.getUInt(_nvs_prog_key_pres(prog), 0); }
uint32_t nvs_get_prog_app(int prog)  { return _nvs.getUInt(_nvs_prog_key_app(prog), 0); }
void nvs_inc_prog_pres(int prog) { _nvs.putUInt(_nvs_prog_key_pres(prog), nvs_get_prog_pres(prog) + 1); }
void nvs_inc_prog_app(int prog)  { _nvs.putUInt(_nvs_prog_key_app(prog),  nvs_get_prog_app(prog)  + 1); }
...
void nvs_fazer_acerto() {
    ...
    // Zera a quebra por origem junto — senão ficava presencial+app > total
    // depois de um acerto (os totais zeram, essa quebra não zerava).
    for (int p = 1; p <= 4; p++) {
        _nvs.putUInt(_nvs_prog_key_pres(p), 0);
        _nvs.putUInt(_nvs_prog_key_app(p), 0);
    }
}
