...
static lv_obj_t* lbl_prog_nome[4]  = {};   // "Prog N"
static lv_obj_t* lbl_prog_pres[4]  = {};   // coluna Presencial
static lv_obj_t* lbl_prog_app[4]   = {};   // coluna App
...
void tela_manual_atualizar_contadores() {
    ...
    for (int i = 0; i < 4; i++) {
        lv_label_set_text_fmt(lbl_prog_pres[i], "%u", (unsigned)nvs_get_prog_pres(i+1));
        lv_label_set_text_fmt(lbl_prog_app[i],  "%u", (unsigned)nvs_get_prog_app(i+1));
    }
...
void tela_manual_criar(void (*cb_config)(), void (*cb_auto)(), void (*cb_pagamento)()) {
    ...
    // Cabeçalho das colunas — Modelo | Presencial | App
    {
        lv_obj_t* h1 = lv_label_create(painel);
        lv_label_set_text(h1, "Modelo");
        lv_obj_set_style_text_color(h1, COR_TEXTO_FRACO, 0);
        lv_obj_set_style_text_font(h1, &lv_font_montserrat_14, 0);
        lv_obj_set_pos(h1, 10, 28);

        lv_obj_t* h2 = lv_label_create(painel);
        lv_label_set_text(h2, "Presencial");
        lv_obj_set_style_text_color(h2, COR_TEXTO_FRACO, 0);
        lv_obj_set_style_text_font(h2, &lv_font_montserrat_14, 0);
        lv_obj_set_pos(h2, 180, 28);

        lv_obj_t* h3 = lv_label_create(painel);
        lv_label_set_text(h3, "App");
        lv_obj_set_style_text_color(h3, COR_TEXTO_FRACO, 0);
        lv_obj_set_style_text_font(h3, &lv_font_montserrat_14, 0);
        lv_obj_set_pos(h3, 300, 28);
    }

    for (int i = 0; i < 4; i++) {
        int y = 48 + i*16;

        lbl_prog_nome[i] = lv_label_create(painel);
        lv_label_set_text_fmt(lbl_prog_nome[i], "Prog %d", i+1);
        lv_obj_set_style_text_color(lbl_prog_nome[i], COR_TEXTO, 0);
        lv_obj_set_pos(lbl_prog_nome[i], 10, y);

        lbl_prog_pres[i] = lv_label_create(painel);
        lv_label_set_text(lbl_prog_pres[i], "0");
        lv_obj_set_style_text_color(lbl_prog_pres[i], COR_TEXTO, 0);
        lv_obj_set_pos(lbl_prog_pres[i], 200, y);

        lbl_prog_app[i] = lv_label_create(painel);
        lv_label_set_text(lbl_prog_app[i], "0");
        lv_obj_set_style_text_color(lbl_prog_app[i], COR_TEXTO, 0);
        lv_obj_set_pos(lbl_prog_app[i], 310, y);
    }

    lv_obj_t* linha = lv_obj_create(painel);
    lv_obj_set_size(linha, 400, 1);
    lv_obj_set_pos(linha, 10, 122);
    ...
    lv_obj_set_pos(lbl_horas, 10, 128);
    ...
    lv_obj_set_pos(lbl_saldo, 10, 146);
