...
static String _backend_estado_str() {
    // Carro fisicamente presente (X14 entrando ou X15 na posição) -> NUNCA
    // reporta FREE pro app, mesmo que a máquina internamente já tenha
    // voltado pra IDLE/CONCLUIDO/ERRO. Achado em teste real: lavagem
    // terminou (ou deu erro) com o carro ainda parado em cima, e o app
    // mostrava "disponível" — um próximo cliente reservaria uma vaga
    // ocupada de verdade. X14/X15 são lidos direto do sensor (nível ao
    // vivo), então refletem a realidade física, não o que a máquina "acha"
    // que já processou.
    if (io2_get_di(5) || io2_get_di(6)) return "WASHING";   // X14 || X15
...
static void _backend_processar_start() {
    ...
    ir_tela_auto_se_preciso();   // leva a tela pra TELA_AUTO (fica lá se já estiver)
...
static void _backend_flush_fila() {
    ...
    strncpy(m.errcode, e.c_str(), sizeof(m.errcode) - 1);
    esp_now_send((uint8_t*)MAC_CAMERA, (uint8_t*)&m, sizeof(m));
...
// Enfileira fault. errorCode: descrição/código do erro (ex: msg_alarme de
// auto_erro()) — SEM isso o backend recebia um fault "vazio" (só
// reservationId, sem saber o que aconteceu) e o app não tinha o que
// mostrar pro usuário.
void backend_evt_fault(const char* errorCode = "") {
    String resId = nvs_get_last_res_id();
    String ec = String(errorCode);
    ec.replace("\"", "'");   // nunca quebrar o JSON por causa de aspas na msg
    String json = "{\"t\":\"fault\"";
    if (resId.length() > 0) json += ",\"r\":\"" + resId + "\"";
    if (ec.length() > 0)    json += ",\"e\":\"" + ec + "\"";
    json += "}";
    nvs_evt_push(json);
    Serial.printf("[EVT] fault enfileirado (erro=%s)\n", errorCode);
}
// -----------------------------------------------------------------------
