...
// Chamado por backend_client.h quando a nuvem autoriza uma lavagem (start):
// leva a tela pra TELA_AUTO (pra mostrar o programa selecionado rodando).
// navegar_para() já é no-op se já estiver lá — não sai e volta à toa.
void ir_tela_auto_se_preciso() { navegar_para(TELA_AUTO); }
...
// -----------------------------------------------------------------------
// Instrumentação de memória — log de heap por etapa do boot (pra medir
// quanto cada tela consome antes de otimizar mais). NOTA: com
// LV_MEM_CUSTOM=1 (lv_conf.h redireciona o malloc do LVGL pra PSRAM),
// lv_mem_monitor() não faz a varredura real do pool interno do LVGL — esse
// código só existe no branch #if LV_MEM_CUSTOM==0 do lv_mem.c da lib. Os
// campos do LVGL abaixo (used/free/frag) vêm ZERADOS por isso; quem carrega
// o número real são os heap_caps_get_free_size() de INTERNAL/PSRAM.
// -----------------------------------------------------------------------
static void log_mem(const char *label) {
    lv_mem_monitor_t mon;
    lv_mem_monitor(&mon);
    Serial.printf("[MEM] %-20s | LVGL used=%6u free=%6u frag=%3u%% | "
                  "INTERNAL free=%8u | PSRAM free=%8u\n",
                  label,
...
void setup() {
    ...
    // com a alimentacao ja estavel.
    // TESTE DE ISOLAMENTO: desativado por completo pra confirmar se e a causa
    // do travamento no boot (tela branca).
    // hard_reset_fase1("DISPLAY");
    // NAO chamar hard_reset_radio() aqui: WiFi.mode(WIFI_OFF) mexe no PLL/clock
    // que o painel RGB precisa pra inicializar (handshake trava = tela branca).
    // Movido pra depois do LCD/LVGL estarem de pe (ver abaixo).
    ...
    // Radio do zero (ESP-NOW/Wi-Fi desligados e zerados) — só agora, com o
    // LCD/LVGL já de pé, pra não interferir no PLL/clock do painel RGB.
    // hard_reset_radio();  // TESTE DE ISOLAMENTO: desativado

    // Inicializa RS-485 (display+2 waveshares+inversor no mesmo barramento;
    // vfd_reset: inversor PARADO e freq zerada) + ESP-NOW (só a câmera agora)
    modbus_init();
    comm_espnow_init();
    ...
    log_mem("after_screen_1_manual");

    log_mem("before_screen_2_auto");
    tela_auto_criar(cb_ir_manual);
    log_mem("after_screen_2_auto");

    log_mem("before_screen_3_config");
    tela_config_criar(cb_ir_manual, cb_ir_velocidades, cb_ir_modelos, cb_ir_alterar_senhas);
    log_mem("after_screen_3_config");

    log_mem("before_screen_4_velocidades");
    tela_velocidades_criar(cb_ir_config);
    log_mem("after_screen_4_velocidades");

    log_mem("before_screen_5_modelos");
    tela_modelos_criar(cb_ir_config);
    log_mem("after_screen_5_modelos");

    log_mem("before_screen_6_senha");
    tela_senha_criar();
    log_mem("after_screen_6_senha");

    log_mem("before_screen_7_senhas_cfg");
    tela_senhas_cfg_criar(cb_ir_config);
    log_mem("after_screen_7_senhas_cfg");

    log_mem("before_screen_8_wifi");
...
// -----------------------------------------------------------------------
void loop() {
    // Consulta o inversor (freq/corrente/status) a cada 500ms — era 100ms.
    // So roda quando ocioso (modbus_tick ja suspende isso 100% durante giro/
    // home/processo, sem mudanca ali); no ocioso, 100ms sobrecarregava o
    // barramento RS-485 compartilhado (9600 baud, meio-duplex) e atrasava a
    // leitura dos sensores das waveshares — encontrado ao investigar o botao
    // X1 nao respondendo. E so telemetria (mostrada na tela), nao precisa
    // ser tao frequente quanto os sensores.
    static unsigned long t_modbus = 0;
    if (millis() - t_modbus >= 500) {
        t_modbus = millis();
        modbus_tick();
    }
    ...
    if (millis() - t_dbg >= 1000) {
        t_dbg = millis();
        Serial.printf("[DBG] X10=%d X0=%d | Y4=%d Y14=%d vfd=%d | X15=%d X16=%d X17=%d X12=%d | rec=%d auto=%d | wave_ok=%d hb1_ms=%lu hb2_ms=%lu | heap=%u max_bloco=%u\n",
