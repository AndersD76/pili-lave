...
static int giro_tick() {
    ...
    // ---- RAMPA — comeca em GIRO_INICIO_RAMPA_MS, contado SEMPRE a partir de
    //      T=0 (Y4 ligou), igual nos dois lados (1->2 e 2->1). Antes, no lado
    //      2->1, contava a partir da SAIDA da pos2 (_giro_t_saiu_pos2, via
    //      X17 zerando) — no giro triplo da Alta Pressao, o giro que termina
    //      na pos1 gira no sentido HORARIO (unico caso do sistema), e o X17
    //      pode nao zerar de forma confiavel nesse sentido -> a rampa nunca
    //      comecava. Usar sempre dt (relogio unico, sem depender de sensor)
    //      resolve isso; a PARADA continua sendo so pelo X16+X17, sem mudanca.
    uint32_t dt_rampa = dt;
...
static uint32_t _cr_t_pulso = 0;   // watchdog X0: ultimo instante em que o X0 pulsou
...
static void carr_rev_x10_iniciar(uint16_t freq_hz10) {
    ...
    _cr_t_x10_on = 0;
    _cr_t_pulso  = millis();              // arma o watchdog do X0 (carro travado no meio-retorno)
    _cr_x0_ant   = X0;
}

// SEM watchdog aqui era um buraco de segurança: se o X10 nunca confirmasse
// ativo (falha de comunicação, sensor, ou carrinho travado de verdade), essa
// sub-máquina ficava em SUB_RUN pra sempre — o giro 2->1 nunca era chamado, e
// o carrinho ficava voltando indefinidamente sem erro nenhum na tela. As
// duas sub-máquinas parecidas (carr_fwd_tick/carr_rev_x12_tick) já tinham
// esse watchdog de X0; faltava só nesta.
static int carr_rev_x10_tick() {
    for (uint8_t k = 0; k < 5; k++) {
        bool x0 = X0;
        if (x0 != _cr_x0_ant) { _cr_t_pulso = millis(); _cr_x0_ant = x0; }
        if (k < 4) modbus_refresh_io1_di();
    }
    if (millis() - _cr_t_pulso > CARRO_WD_MS) {   // X0 sem pulsar -> carro travado (ou sem comunicacao)
        vfd_stop(); SET_Y14(false);
        _cf_msg = "CARRO TRAVADO (retorno X10)";
        return SUB_ERR;
    }
...
enum {
    ...
    SP_REV_INI, SP_REV, SP_FIM, SP_ESPERA_FIM
};
...
static bool _proc_simples(bool com_giro, uint32_t fwd_dwell,
                          ...
                          uint32_t espera_fim_ms = 0,
                          void (*fim_liga)() = nullptr,
                          void (*fim_desliga)() = nullptr) {
    switch (_pst) {
        case SP_HOME:
            // TESTE: checagem de HOME por processo desativada — validacao unica agora
            // no fim da entrada de carros (AUTO_INICIANDO). Descomentar p/ reverter.
        ...
        case SP_FIM:
            desliga();
            if (espera_fim_ms > 0) {
                if (fim_liga) fim_liga();   // ex.: Cera de Agua liga Y0 por 2s no final
                _pt = millis(); _pst = SP_ESPERA_FIM;
            } else { _pst = 0; return true; }
            break;

        // Espera final, DEPOIS de desligar os solenoides — segura a transicao
        // pro proximo processo da fila (ex.: Espuma A = 30s, Espuma B = 60s,
        // Cor Magica = 45s). fim_liga/fim_desliga (opcionais) ligam algo SO
        // durante essa espera (ex.: Cera de Agua liga Y0 os 2s inteiros).
        case SP_ESPERA_FIM:
            if (millis() - _pt >= espera_fim_ms) {
                if (fim_desliga) fim_desliga();
                _pst = 0; return true;
            }
            break;
    }
...
// fwd_dwell = 2000 (era 1000): anda mais 1s depois de perder o X10 antes de
...
//      No final: liga Y0 por 2s continuos antes de concluir (pedido do
//      operador — mesmo Y0 ja usado na Cor Magica).
