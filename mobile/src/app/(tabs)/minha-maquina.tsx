          ...
          return (
            ...
            >
              ...
              <View style={{ marginTop: 12, gap: 4 }}>
                ...
                {(() => {
                  const totalPresLav = m.porTipo.reduce((s, t) => s + t.presencial.lavagens, 0);
                  const totalPresCents = m.porTipo.reduce((s, t) => s + t.presencial.valorCents, 0);
                  const totalAppLav = m.porTipo.reduce((s, t) => s + t.app.lavagens, 0);
                  const totalAppCents = m.porTipo.reduce((s, t) => s + t.app.valorCents, 0);
                  return (
                    <View style={{ flexDirection: "row", paddingTop: 8, marginTop: 4, borderTopWidth: 1, borderTopColor: C.linha, alignItems: "center" }}>
                      <Text style={{ flex: 1, fontFamily: F.bodyBold, fontSize: 13, color: C.cromo }}>Total</Text>
                      <View style={{ flex: 1, alignItems: "flex-end" }}>
                        <Text style={{ fontFamily: F.body, fontSize: 12, color: C.acoD }}>{totalPresLav}x</Text>
                        <Text style={{ fontFamily: F.displayX, fontSize: 14, color: C.cromo }}>{money(totalPresCents)}</Text>
                      </View>
                      <View style={{ flex: 1, alignItems: "flex-end" }}>
                        <Text style={{ fontFamily: F.body, fontSize: 12, color: C.acoD }}>{totalAppLav}x</Text>
                        <Text style={{ fontFamily: F.displayX, fontSize: 14, color: C.cromo }}>{money(totalAppCents)}</Text>
                      </View>
                    </View>
                  );
                })()}
